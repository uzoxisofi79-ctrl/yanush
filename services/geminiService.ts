...
export async function analyzeChatSession(
  ...
  const instruction = `
Ты — комиссия по оценке педагогического диалога учителя с подростком.
Акцентуация подростка: ${accentuation}
Причина завершения сессии: ${reason}

ЭКСПЕРТНАЯ КОМИССИЯ (8 специалистов — каждый ОБЯЗАН присутствовать в ответе):
1. Клинический психолог — оценивает распознавание эмоционального состояния, адекватность реакций педагога
2. Психиатр — оценивает понимание психотипа, риски эскалации, соответствие действий клинической картине
3. Семейный психолог — оценивает учёт семейного контекста, тактичность в вопросах семьи
4. Криминолог — оценивает распознавание манипуляций, провокаций, скрытых мотивов
5. Социальный педагог — оценивает соблюдение прав ребёнка, этические границы, профессиональную позицию
6. Медиатор — оценивает навыки деэскалации, умение вести переговоры, снижение напряжения
7. Подростковый психолог — оценивает понимание подростковой психологии, возрастных особенностей
8. Юрист — оценивает правовую корректность действий, соблюдение законодательства об образовании

СОВЕЩАТЕЛЬНАЯ КОМИССИЯ (6 шаржированных персонажей — моделируют общественное давление, НЕ влияют на итоговую оценку, их мнения должны быть характерны для их персонажей, допустима лёгкая ирония):
1. Олигарх-попечитель — спонсор школы, мыслит «эффективностью» и «репутацией учреждения»
2. Военрук — ветеран, убеждён, что дисциплина решает всё
3. Инстаграм-психолог — сертификат из интернета, говорит про «токсичные установки» и «внутреннего ребёнка»
4. Зумер-блогер — оценивает по критерию «залетит в рекомендации или нет»
5. Тревожная мать — её ребёнок всегда прав, учитель всегда виноват
6. Чиновник из департамента — боится скандала, главное — «чтобы не дошло до прессы»

Ответь ТОЛЬКО СТРОГИМ JSON (без \`\`\`):
{
  "overall_score": число 0..100 (среднее экспертной комиссии),
  "summary": "2-3 предложения — общий итог, профессиональный тон",
  "commission": [
    { "name": "Имя Фамилия", "role": "Специальность", "score": число 0..100, "verdict": "развёрнутый комментарий 2-4 предложения" }
  ],
  "advisory_commission": [
    { "name": "Имя", "role": "Персонаж", "verdict": "характерный комментарий 1-2 предложения в стиле персонажа" }
  ]
}
Никакого текста вне JSON. В commission — 8 экспертов. В advisory_commission — 6 персонажей.
`;
  ...
  try {
    ...
    const advisoryRaw = Array.isArray(parsed?.advisory_commission) ? parsed.advisory_commission : [];
    const advisory_commission = advisoryRaw.map((m: any) => ({
      name: String(m?.name ?? "Персонаж"),
      role: String(m?.role ?? ""),
      verdict: String(m?.verdict ?? ""),
    }));
